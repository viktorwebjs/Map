...
BUSINESS_ENTITY_STATUSES.forEach(({ value, title }) =>
  statusesMap.set(value, title)
);
PAYMENT_PLANS.forEach(({ id, planName }) => paymentMap.set(id, planName));
newUsers = businessEntities.map(({ state, planId, status, name, id }) => {
  return {
    location: state,
    status: statusesMap.get(status),
    planName: paymentMap.get(planId),
    name,
    id,
  };
});
